...
def andrews_films_and_leads
  ...
  FROM
    movies
      JOIN castings ON castings.movie_id = movies.id
      JOIN actors ON castings.actor_id = actors.id
      JOIN castings AS other_casts ON castings.movie_id = other_casts.movie_id
      JOIN actors AS other_actors ON other_casts.actor_id = other_actors.id
  WHERE
    actors.name = 'Julie Andrews' AND other_casts.ord = 1
  SQL
end
...
def films_by_cast_size
  ...
  SELECT
    movies.title, COUNT(*)
  FROM
    movies JOIN castings ON castings.movie_id = movies.id
  WHERE
    movies.yr = 1978
  GROUP BY
    movies.id
